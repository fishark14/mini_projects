...
#         if self.KO:
#             print('{pokemon} cannot attack because it has fainted...'.\
#                  format(pokemon = self.name))
...
{target} received {damage} damage. {target} has {health} HP left.'''.\
                 format(self=self.name, target=target.name,\
...
""".format(name=self.name, main=self.pokemons[self.main_pokemon].name,\
                                 pokemons = [i.name for i in self.pokemons],\
           potions=self.potions)
        
    def use_potion(self):
        if self.potions > 1:
            print('{trainer} used a potion on {pokemon}...'.
                  format(trainer=self.name,\
                         pokemon=self.pokemons[self.main_pokemon].name))
